...
/**
 * INTEGRATION GUIDE: Multi-Strategy Framework
 *
 * The InvestPro trading system now supports multi-strategy management with automatic
 ...
 *
 * This registers all available strategies (TrendFollowing, MeanReversion, Breakout)
 ...
 *
 *    selectionService.manuallyAssign(symbol, timeframe, "trend-following", locked=true, reason);
 *
 ...
 *
 *    selectionService.disableStrategy(symbol, timeframe, "Strategy underperforming");
 *
 *
 * ============================================================================
 ...
 *    List<StrategyBacktestResult> results = selectionService.getBacktestResults(symbol, timeframe);
 *
 *
 * ============================================================================
 ...
 *        System.out.println(entry.strategyId() + " at " + entry.timestamp() + ": " + entry.note());
 *    }
 *
 *
 * ============================================================================
 ...
 *   └── Metrics from backtesting (trades, win rate, drawdown, Sharpe ratio, etc.)
 *
 ...
 * List<StrategyBacktestResult> results = runBacktestForSymbol(symbol, timeframe);
 *
 * // 3. Rank strategies
 * StrategyRankingEngine rankingEngine = new StrategyRankingEngine();
 * List<StrategyScore> rankedStrategies = rankingEngine.rankStrategies(results);
 *
 * // 4. Auto-assign best strategy
 * StrategySelectionService selectionService = StrategySelectionService.getInstance();
 * StrategyAssignment assignment = selectionService.selectAndAssign(symbol, timeframe, results);
 *
 * // 5. Later, in live trading, get the assignment
 * StrategyAssignment current = selectionService.getCurrentAssignment(symbol, timeframe);
 * if (current != null) {
 *     TradingStrategy strategy = StrategyRegistry.getInstance().getStrategy(current.getStrategyId());
 *     StrategySignal signal = strategy.generateSignal(context);
 *     // Use signal...
 * }
 *
 * // 6. Manual override if needed
 * selectionService.manuallyAssign(symbol, timeframe, "mean-reversion", true, "User expertise");
 *
 *
 * ============================================================================
 * NEXT STEPS
 ...
 * 3. ⏳ Build backtesting matrix runner for all symbol/timeframe/strategy combos
