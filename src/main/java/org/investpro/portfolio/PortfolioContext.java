...
public class PortfolioContext {
    ...
    @Nullable
    private final Side candidateSignal;
    ...
    public double getCandidateExposureAfterTrade(double sizePercent) {
        // Rough estimate of exposure change: (requestedPositionSize *
        // requestedLeverage) / accountEquity
        if (accountEquity <= 0)
            return 0;
    ...
    public double getMaxPositionSizeWithMargin() {
        if (requestedLeverage <= 0)
            return 0;
        return (accountEquity * requestedLeverage);
    }
