...
public class BisMarketStructureService implements MarketStructureService {
    ...
    LiquidityTier classifyByCode(@NotNull String base, @NotNull String quote) {
        ...
        if (combinedScore >= 55.0) {
            // Both currencies relatively important but not in static map
            return LiquidityTier.TIER_2_MAJOR_CROSS;
    ...
    private double sessionActivityScore(@NotNull String session) {
        // Returns relative activity level for current session — simplified constant here;
        // callers can enrich with time-of-day logic if needed.
        return switch (session) {
            case "London", "New York", "Overlap" -> 1.00;
            case "Tokyo" -> 0.80;
            case "Sydney" -> 0.60;
            default -> 0.40;
        };
