...
public class ChartWorkspacePane extends VBox {
    ...
    private static final double MIN_DEPTH_CANVAS_WIDTH = 320;
    private static final double MIN_DEPTH_CANVAS_HEIGHT = 260;
    private static final double MAX_DEPTH_CANVAS_DIMENSION = 4096;
    ...
    public ChartWorkspacePane(Exchange exchange,
        ...
        workspaceTabs.getSelectionModel().select(candlestickTab);
    ...
    private Node createDepthPane() {
        ...
        depthPane.widthProperty().addListener((observable, oldWidth, newWidth) -> {
            resizeDepthCanvas(
                    clampDepthCanvasDimension(newWidth.doubleValue() - 28, MIN_DEPTH_CANVAS_WIDTH),
                    depthCanvas.getHeight()
            );
            drawDepthChart(latestOrderBook);
        });
        depthPane.heightProperty().addListener((observable, oldHeight, newHeight) -> {
            resizeDepthCanvas(
                    depthCanvas.getWidth(),
                    clampDepthCanvasDimension(newHeight.doubleValue() - 28, MIN_DEPTH_CANVAS_HEIGHT)
            );
            drawDepthChart(latestOrderBook);
        });
    ...
    private void drawDepthChart(OrderBook orderBook) {
        ...
        if (!isDepthCanvasRenderable(width, height)) {
            return;
        }
    ...
    private void resizeDepthCanvas(double width, double height) {
        double safeWidth = clampDepthCanvasDimension(width, MIN_DEPTH_CANVAS_WIDTH);
        double safeHeight = clampDepthCanvasDimension(height, MIN_DEPTH_CANVAS_HEIGHT);
        if (Double.compare(depthCanvas.getWidth(), safeWidth) != 0) {
            depthCanvas.setWidth(safeWidth);
        }
        if (Double.compare(depthCanvas.getHeight(), safeHeight) != 0) {
            depthCanvas.setHeight(safeHeight);
        }
    }

    private double clampDepthCanvasDimension(double proposedDimension, double minimumDimension) {
        if (!Double.isFinite(proposedDimension)) {
            return minimumDimension;
        }
        return Math.max(minimumDimension, Math.min(proposedDimension, MAX_DEPTH_CANVAS_DIMENSION));
    }

    private boolean isDepthCanvasRenderable(double width, double height) {
        return Double.isFinite(width)
                && Double.isFinite(height)
                && width >= MIN_DEPTH_CANVAS_WIDTH
                && height >= MIN_DEPTH_CANVAS_HEIGHT
                && width <= MAX_DEPTH_CANVAS_DIMENSION
                && height <= MAX_DEPTH_CANVAS_DIMENSION;
    }
