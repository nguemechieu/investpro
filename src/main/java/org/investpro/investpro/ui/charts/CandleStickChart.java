...
public class CandleStickChart extends Region {
    ...
    private static final Color RSI_COLOR = Color.web("#f97316", 0.95);
    private static final Color MACD_COLOR = Color.web("#22d3ee", 0.95);
    private static final Color STOCHASTIC_COLOR = Color.web("#84cc16", 0.95);
    private static final Color OSCILLATOR_GRID = Color.web("#94a3b8", 0.16);
    ...
    public CandleStickChart(Exchange exchange, TradePair tradePair, CandleDataSupplier candleDataSupplier,
        ...
        chartOptions.horizontalGridLinesVisibleProperty().addListener((observable, oldValue, newValue) -> drawChartContents(true));
        chartOptions.verticalGridLinesVisibleProperty().addListener((observable, oldValue, newValue) -> drawChartContents(true));
        ...
        chartOptions.showSma20Property().addListener((observable, oldValue, newValue) -> drawChartContents(true));
        chartOptions.showEma50Property().addListener((observable, oldValue, newValue) -> drawChartContents(true));
        chartOptions.showBollingerBandsProperty().addListener((observable, oldValue, newValue) -> drawChartContents(true));
        chartOptions.showRsi14Property().addListener((observable, oldValue, newValue) -> drawChartContents(true));
        chartOptions.showMacdProperty().addListener((observable, oldValue, newValue) -> drawChartContents(true));
        chartOptions.showStochastic14Property().addListener((observable, oldValue, newValue) -> drawChartContents(true));
    ...
    private void setupCanvasAndLayout(ObservableNumberValue containerWidth, ObservableNumberValue containerHeight) {
            ...
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                ...
                CompletableFuture
                        ...
                        .exceptionally(e -> {
                            ...
                            showChartStatus("Failed to load candle data: " + rootCauseMessage(e));
                            return null;
                        });
            }
        };
        ...
        Platform.runLater(() -> {
            if (!disposed && !gotFirstSize.get()
                    && containerWidth.doubleValue() > 0
                    && containerHeight.doubleValue() > 0) {
                gotFirstSize.set(true);
            }
        });
    }
        ...
        if (desiredXLowerBound <= maxBound) {
            if (needsPaging) {
                ...
                CompletableFuture
                        .supplyAsync(candleDataSupplier)
                        .thenAccept(candleDataPager.getCandleDataPreProcessor())
                        .whenComplete((result, throwable) -> runLaterIfAlive(() -> {
                            if (throwable != null) {
    ...
    private void drawIndicatorOverlays(List<CandleData> visibleCandles, int startingCandleIndex) {
        if (!chartOptions.isShowSma20()
                && !chartOptions.isShowEma50()
                && !chartOptions.isShowBollingerBands()
                && !chartOptions.isShowRsi14()
                && !chartOptions.isShowMacd()
                && !chartOptions.isShowStochastic14()) {
            return;
        }
        ...
        if (chartOptions.isShowRsi14()) {
            drawBoundedOscillatorLine(
                    visibleCandles,
                    candleIndexByOpenTime,
                    IndicatorCalculator.calculateRSISeries(orderedCandles, 14),
                    startingCandleIndex,
                    RSI_COLOR,
                    "RSI 14",
                    12
        ...
        if (chartOptions.isShowMacd()) {
            drawFloatingOscillatorLine(
                    visibleCandles,
                    candleIndexByOpenTime,
                    IndicatorCalculator.calculateMACDSeries(orderedCandles),
                    startingCandleIndex,
                    MACD_COLOR,
                    "MACD",
                    74
        ...
        if (chartOptions.isShowStochastic14()) {
            drawBoundedOscillatorLine(
                    visibleCandles,
                    candleIndexByOpenTime,
                    IndicatorCalculator.calculateStochasticSeries(orderedCandles, 14),
                    startingCandleIndex,
                    STOCHASTIC_COLOR,
                    "Stoch 14",
                    126
    ...
    private void drawBoundedOscillatorLine(List<CandleData> visibleCandles,
                                           Map<Integer, Integer> candleIndexByOpenTime,
                                           List<Double> series,
                                           int startingCandleIndex,
                                           Color color,
                                           String label,
                                           double labelX) {
        drawOscillatorLine(visibleCandles, candleIndexByOpenTime, series, startingCandleIndex, color, label, labelX, 0, 100);
    }

    private void drawFloatingOscillatorLine(List<CandleData> visibleCandles,
                                            Map<Integer, Integer> candleIndexByOpenTime,
                                            List<Double> series,
                                            int startingCandleIndex,
                                            Color color,
        ...
        drawOscillatorLine(visibleCandles, candleIndexByOpenTime, series, startingCandleIndex, color, label, labelX, min, max);
    }

    private void drawOscillatorLine(List<CandleData> visibleCandles,
                                    Map<Integer, Integer> candleIndexByOpenTime,
                                    List<Double> series,
                                    int startingCandleIndex,
                                    Color color,
                                    ...
                                    double minValue,
                                    double maxValue) {
        double panelHeight = Math.min(92, Math.max(54, canvas.getHeight() * 0.18));
        double panelBottom = canvas.getHeight() - 16;
        ...
        graphicsContext.save();
        graphicsContext.setStroke(OSCILLATOR_GRID);
        graphicsContext.setLineWidth(1);
        graphicsContext.strokeLine(0, panelTop, canvas.getWidth(), panelTop);
        graphicsContext.strokeLine(0, panelBottom, canvas.getWidth(), panelBottom);
        graphicsContext.setFill(color);
        graphicsContext.setFont(BADGE_FONT);
        graphicsContext.fillText(label, labelX, panelTop + 14);

        graphicsContext.setStroke(color);
        graphicsContext.setLineWidth(1.45);
        ...
        for (int i = 0; i < visibleCandles.size(); i++) {
            ...
            double normalized = Math.max(0, Math.min(1, (value - minValue) / range));
            int relativeFromLatest = visibleCandles.size() - 1 - i;
            int candleSlot = startingCandleIndex + relativeFromLatest;
            double x = candleCenterX(candleSlot);
            double y = panelBottom - (normalized * panelHeight);
            if (!pathStarted) {
                graphicsContext.beginPath();
                graphicsContext.moveTo(x, y);
                pathStarted = true;
        ...
        CompletableFuture.runAsync(() -> {
            try {
                if (TradingSession.isPaperTrading()) {
                    runLaterIfAlive(() -> showChartNotification(String.format(
                            "Paper %s %s order staged: %.4f @ %s",
                            side.name(),
                            orderType.name(),
                            size,
    ...
    private void setupCrosshairOverlay() {
        ...
        canvas.setOnMouseExited(event -> hideCrosshair());
    ...
    private void showChartNotification(String message) {
        runLaterIfAlive(() -> {
            ...
            chartNotificationTransition.setOnFinished(event -> hideChartStatus());
        ...
        private final class CandleDataPreProcessor implements Consumer<Future<List<CandleData>>> {
            ...
            public void accept(@NotNull Future<List<CandleData>> futureCandleData) {
                ...
                try {
                    ...
                    showChartStatus("Failed to load candle data: " + rootCauseMessage(ex));
                    return;
                }

                if (candleData0 == null || candleData0.isEmpty()) {
                    logger.warn("No candle data returned for {}", tradePair);
                    showChartStatus("No candle data returned for " + tradePair.toString('/') + ".");
                    return;
                }
                ...
                int count = 0;
                while (count < candleData0.size() && candleData0.get(count).isPlaceHolder()) {
                    count++;
                }
                ...
                if (!nonPlaceHolders.isEmpty()) {
                    ...
                    showChartStatus("No real candle data found for " + tradePair.toString('/') + ".");
                }
