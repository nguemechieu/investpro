...
public class CandleStickChartContainer extends Region {
    ...
    private ChartLayout candleStickChart;
    ...
    public CandleStickChartContainer(Exchange exchange, TradePair tradePair) {
        ...
        secondsPerCandle.addListener((observableDurationValue, oldDurationValue, newDurationValue) -> {
            if (!oldDurationValue.equals(newDurationValue)) {
                createNewChart(newDurationValue.intValue(), true);
                toolbar.registerEventHandlers(candleStickChart, secondsPerCandle);
    ...
    public CandleStickChartContainer(org.investpro.investpro.@NotNull Exchange exchange, TradePair selectedPair, boolean liveSyncing) {
    }
    ...
    private void createNewChart(int secondsPerCandle, boolean liveSyncing) {
        ...
        */
        try {
            ...
            candleStickChart = new ChartLayout(exchange, tradePair, exchange.getCandleDataSupplier(secondsPerCandle, tradePair),
                    liveSyncing, secondsPerCandle, widthProperty(), heightProperty(), new TelegramClient(tokens));
        } catch (Exception e) {
