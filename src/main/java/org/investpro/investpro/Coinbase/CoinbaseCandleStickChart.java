 ...
 * a {@link CoinbaseCandleStickChartContainer}. To enforce this usage, the constructors for this class are package-private.
 * <p>
 ...
 */
public class CoinbaseCandleStickChart extends Region {
    private static final DecimalFormat MARKER_FORMAT = new DecimalFormat("#.000");
    private final CoinbaseCandleDataPager candleDataPager;
    private final CoinbaseCandleStickChartOptions chartOptions;
    /**
    ...
    private double chartWidth = 1500;
    private double chartHeight = 800;
     ...
     * be instantiated by a {@link CoinbaseCandleStickChartContainer}.
     *
     ...
     */
 public CoinbaseCandleStickChart(Exchange exchange, Coinbase.CoinbaseCandleDataSupplier candleDataSupplier, String tradePair,
     ...
     candleDataPager = new CoinbaseCandleDataPager(this,candleDataSupplier);
        data = Collections.synchronizedNavigableMap(new TreeMap<>(Integer::compare));
        chartOptions = new CoinbaseCandleStickChartOptions();
        ...
        yAxis.setTickLabelFormatter(new MoneyAxisFormatter(org.investpro.investpro.Currency.of(tradePair.substring(4, tradePair.length() - 1))));
        extraAxis.setTickLabelFormatter(new MoneyAxisFormatter(org.investpro.investpro.Currency.of(tradePair.substring(4, tradePair.length() - 1))));
        ...
        symbolLabel.setText(tradePair + "  " + org.investpro.investpro.Currency.of(tradePair.substring(0, 3)) + " " + Currency.of(tradePair.substring(0, 3)).getSymbol());
        ...
        ChangeListener<? super Boolean> gotFirstSizeChangeListener = new ChangeListener<>() {
            ...
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
               ...
               // chartWidth = (Math.floor(containerWidth.getValue().doubleValue() / candleWidth) * candleWidth) - 10 +
                ...
                list0.addAll(  Coinbase.getOrders());
    ...
    private void CreateOrder(String side, double size, @NotNull PLATFORM exchangeName) throws OandaException {
        ...
        switch (exchangeName) {
            ...
            case OANDA -> Coinbase.createMarketOrder(tradePair, "MARKET", side, (int) size);
    ...
    private void layoutChart() {
        ...
        xAxis.setPrefSize(chartWidth - 10, xAxisHeight);
        yAxis.setPrefSize(yAxisWidth, chartHeight - 10);
        extraAxis.setPrefSize(yAxisWidth, (chartHeight - 10) * 0.25);
        xAxis.setLayoutY(chartHeight);
        yAxis.setLayoutY(top);
        extraAxis.setLayoutX(chartWidth - 10);
        extraAxis.setLayoutY((chartHeight - 10) * 0.75);
    ...
    public CoinbaseCandleStickChartOptions getChartOptions() {
        return chartOptions;
    }
    ...
    public boolean drawMarketNews(@NotNull News news) {

        Canvas newsCanvas = new Canvas();

        Line line = new Line();
        line.setStartY(chartHeight);
        line.setStartX(chartWidth);
        if (Objects.equals(news.getImpact(), "high")) {
            line.setStartX(chartWidth - 20);
            line.setStartY(chartHeight);
        ...
        if (news.getImpact().equals("medium")) {
            line.setStartX(chartWidth - 20);
            line.setStartY(chartHeight);
...
//            }
            ...
            if (data.isEmpty()) {
                if (liveSyncing) {
                    ...
                    } catch (OandaException | InterruptedException e) {
                        throw new RuntimeException(e);
                    }
