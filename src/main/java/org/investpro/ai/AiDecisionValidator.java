 ...
 * - AI cannot approve if capital protection is NONE and psychology is IMPULSIVE/FEARFUL
 * - Invalid AI output becomes ESCALATE_TO_MANUAL_REVIEW
 *
 * The AI layer operates within these constraints, and any violation results in safe degradation.
 */
public class AiDecisionValidator {
    
    private static final Logger logger = LoggerFactory.getLogger(AiDecisionValidator.class);
     ...
     */
    public static AiTradeReviewResponse validateAndEnforce(AiTradeReviewRequest request, AiTradeReviewResponse aiResponse) {
        ...
        // 5. Check: Cannot approve if capital protection is NONE and psychology is IMPULSIVE/FEARFUL
        if (hasCapitalProtectionRisk(request)) {
            if (aiResponse.getDecision() == AiDecision.APPROVE) {
                ...
                validatedResponse = escalateToManualReview(aiResponse, "Capital protection risk requires manual review");
            }
        }
    ...
    /**
     * Check if there's a capital protection risk (no protection + impulsive/fearful psychology).
     */
    ...
    private static boolean isResponseValid(AiTradeReviewResponse response) {
        ...
        if (Double.isNaN(response.getConfidence()) || response.getConfidence() < 0.0 || response.getConfidence() > 1.0) {
            return false;
        }
     ...
     */
    private static AiTradeReviewResponse overrideExecutionStrategy(@NotNull AiTradeReviewResponse response, String newStrategy) {
        return AiTradeReviewResponse.builder()
                .decision(response.getDecision())
                .confidence(response.getConfidence())
                .suggestedRiskMultiplier(response.getSuggestedRiskMultiplier())
