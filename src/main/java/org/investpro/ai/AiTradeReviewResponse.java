...
/**
 * Immutable response object returned by AiReasoningService after reviewing a trade.
 * Contains the AI's decision, confidence level, and detailed reasoning.
 *
 * The AI NEVER places trades directly. This response feeds into FinalRiskGate for final authority.
 */
@Value
@Builder
@Getter
...
public class AiTradeReviewResponse {
    ...
    /** AI decision: APPROVE, APPROVE_WITH_REDUCED_SIZE, WAIT, REJECT, or ESCALATE_TO_MANUAL_REVIEW */
    AiDecision decision;
    ...
    /** Suggested risk multiplier (0.5 = reduce to 50% of normal, 1.0 = keep as-is, >1.0 not allowed) */
    double suggestedRiskMultiplier;
    
    /** Suggested position size (if reduced, must be <= RiskDecision.finalPositionSize) */
     ...
     * Used when API is unavailable, request is malformed, or AI returns invalid data.
     */
