...
final class BinanceTransferProvider implements TransferProvider {

    private final Binance binance;
    private final Map<String, TransferStatus> statuses = new LinkedHashMap<>();

    BinanceTransferProvider(Binance binance) {
    ...
    public Map<String, BigDecimal> getBalances() {
        try {
            Account account = binance.getUserAccountDetails();
    ...
    public List<String> getSupportedCurrencies() {
        ...
        return List.of("USDT", "USDC", "BTC", "ETH", "SOL", "BNB", "XRP", "LTC");
    }
    ...
    public TransferValidator.ValidationOutcome validateTransfer(TransferRequest request) {
        ...
        if (binance == null) {
            return new TransferValidator.ValidationOutcome(false, List.of("Binance exchange is unavailable."),
                    List.of());
        }
        if (binance.isPaperTrading()) {
            return new TransferValidator.ValidationOutcome(false,
                    List.of("Binance paper mode cannot execute live transfers."), List.of());
        }
        ...
        if ("deposit".equals(hints.action())) {
            return new TransferValidator.ValidationOutcome(false,
                    List.of("Binance API transfer adapter supports crypto withdrawals only."), List.of());
        }

        if (!isCryptoCurrency(request.currency())) {
            return new TransferValidator.ValidationOutcome(false,
                    List.of("Binance transfer supports crypto assets only."), List.of());
        }

        if (hints.cryptoAddress().isBlank()) {
            return new TransferValidator.ValidationOutcome(false,
                    List.of("For Binance transfer, add crypto_address=<address> in Notes."), List.of());
        }
    ...
    public TransferResult executeTransfer(TransferRequest request) {
        ...
        if (!validation.valid()) {
            return new TransferResult("BN-REJECTED", request, TransferStatus.FAILED,
                    String.join("; ", validation.errors()), fee, netAmount, "Rejected");
        }

        RoutingHints hints = parseRoutingHints(request.notes());
        try {
            String response = binance.requestWithdrawalToCryptoAddress(
                    request.amount(),
                    request.currency(),
                    hints.cryptoAddress(),
                    hints.network(),
            ...
            return new TransferResult(transactionId, request, TransferStatus.PENDING,
                    "Submitted to Binance", fee, netAmount, "Pending Binance settlement");
        } catch (Exception exception) {
            String message = exception.getMessage() == null ? "Binance transfer failed" : exception.getMessage();
            return new TransferResult(
                    "BN-FAILED-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase(Locale.ROOT),
                    request,
    ...
    private boolean isCryptoCurrency(String currency) {
        ...
        return switch (currency.toUpperCase(Locale.ROOT)) {
            case "BTC", "ETH", "SOL", "USDC", "USDT", "XRP", "LTC", "ADA", "DOGE", "BNB" -> true;
            default -> false;
        };
    }
    ...
    private String extractReference(String responseBody) {
        if (responseBody == null || responseBody.isBlank()) {
            return "BN-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase(Locale.ROOT);
        }
