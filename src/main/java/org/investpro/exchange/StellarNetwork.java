...
public class StellarNetwork extends Exchange {
    ...
    // EURC (Euro Coin) issuer — Circle — mainnet and testnet
    ...
    public boolean supportsTradePair(TradePair tradePair) {
        if (tradePair == null) return false;
        String base = tradePair.getBaseCurrency().getCode().toUpperCase();
        String quote = tradePair.getCounterCurrency().getCode().toUpperCase();
        // Native XLM pairs always supported
        if ("XLM".equals(base)) return true;
        // Non-native stablecoin pairs supported when both issuers are known
        return !resolveIssuer(base).isEmpty() && !resolveIssuer(quote).isEmpty();
    }
    ...
    private String eurcIssuer() {
        return isPaperTrading() ? TESTNET_EURC_ISSUER : MAINNET_EURC_ISSUER;
    }

    /** Resolves the Stellar issuer address for a known asset code, or returns empty string. */
    private String resolveIssuer(String assetCode) {
        if (assetCode == null) return "";
        return switch (assetCode.toUpperCase()) {
            case "USDC", "USD" -> usdcIssuer();
            case "EURC" -> eurcIssuer();
            default -> "";
        };
    }
    ...
    private TradePair parseSymbolOrDefault(String symbol) {
        ...
        try {
            ...
            if (parts.length == 2) {
                return new TradePair(parts[0].trim(), parts[1].trim());
            }
    ...
    private Ticker safeTicker(TradePair pair) {
        TradePair safePair = pair == null ? defaultPair() : pair;
        try {
            OrderBook book = fetchStellarOrderBook(safePair);
            double bid = book.getBestBid() == null ? 0.0 : book.getBestBid().getPrice();
            double ask = book.getBestAsk() == null ? 0.0 : book.getBestAsk().getPrice();
            if (bid <= 0 && ask <= 0) {
            ...
            ticker.setTradePair(safePair);
            return ticker;
        } catch (Exception e) {
            log.debug("Unable to fetch Stellar ticker for {}: {}", safePair, e.getMessage());
    ...
    private List<Trade> fetchRecentTrades(TradePair tradePair, int limit) {
        ...
        try {
            JSONObject json = httpGetJson("%s/trades?%s&order=desc&limit=%d".formatted(horizonUrl(), assetQuery(pair), safeLimit));
            JSONArray records = json.optJSONArray("_embedded") != null
                    ? json.getJSONObject("_embedded").optJSONArray("records")
            ...
            for (int i = 0; i < records.length(); i++) {
                ...
                if (r != null) {
                    double price = r.optDouble("price", DEFAULT_XLM_USDC_PRICE);
                    double amount = r.optDouble("base_amount", 0.0);
                    String ledgerCloseTime = r.optString("ledger_close_time", null);
                    Instant ts = ledgerCloseTime != null ? Instant.parse(ledgerCloseTime) : Instant.now();
                    trades.add(new Trade(pair, price, amount, Side.BUY, nextOrderId++, ts));
                }
            }
            return trades;
        } catch (Exception e) {
    ...
    private String assetQuery(TradePair pair) {
        String baseCode  = pair.getBaseCode();
        String quoteRaw  = pair.getCounterCode();
        String quoteCode = "USD".equalsIgnoreCase(quoteRaw) ? "USDC" : quoteRaw;

        String buyingIssuer = resolveIssuer(quoteCode);

        if ("XLM".equalsIgnoreCase(baseCode)) {
            // Native XLM selling — standard Stellar order-book query
        ...
        // Non-native base (e.g. USDC/EURC): both sides are credit_alphanum4
        String sellingIssuer = resolveIssuer(baseCode);
        if (sellingIssuer.isEmpty() || buyingIssuer.isEmpty()) {
            throw new IllegalArgumentException(
                    "Missing Stellar issuer for pair: " + baseCode + "/" + quoteCode);
        }
        return "selling_asset_type=credit_alphanum4"
                + "&selling_asset_code=" + baseCode
