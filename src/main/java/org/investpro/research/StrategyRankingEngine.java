 ...
 * Uses multi-factor risk-adjusted scoring: 35% profit + 25% risk + 20% consistency + 10% execution + 10% stability.
 */
@Slf4j
public class StrategyRankingEngine {
    ...
    /**
     * Scores multiple backtest results and returns them ranked by total score (highest first).
     */
    @NotNull
    ...
    private double calculateRiskScore(@NotNull StrategyBacktestResult result) {
        ...
        double maxLossScore = result.getLargestLoss() > 0 ? Math.max(0, 100 - (Math.abs(result.getLargestLoss()) / 2.0)) : 100;
    ...
    private List<String> buildReasons(@NotNull StrategyBacktestResult result, double profit, double risk, double consistency) {
