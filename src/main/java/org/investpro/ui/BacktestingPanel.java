...
import org.investpro.risk.RiskReport;
...
/**
 * BacktestingPanel - Advanced backtesting with comprehensive risk management
 * framework
 * Asks: What market? What trader? What risk? What execution? What capital? What
...
public class BacktestingPanel extends VBox {
    ...
    private VBox createResultsSection() {
        ...
        resultsArea
                .setStyle("-fx-control-inner-background: #f5f5f5; -fx-font-family: 'Courier New'; -fx-font-size: 10;");
        resultsArea.setText("═════════════════════════════════════════════════════════\n" +
    ...
    private VBox createRiskManagementPanel() {
        ...
        liquidityCombo.setValue(LiquidityProfile.NORMAL);
    ...
    private VBox createRiskAnalysisPanel() {
        ...
        Button riskReportButton = new Button("Generate Risk Report");
        riskReportButton.setPrefWidth(200);
        riskReportButton.setStyle(
                "-fx-padding: 10px 20px; -fx-font-size: 12; -fx-background-color: #3b82f6; -fx-text-fill: white;");
        riskReportButton.setOnAction(event -> {
            try {
                displayRiskReport();
    ...
    private TradeRiskContext buildTradeRiskContext(double initialBalance) throws SQLException, ClassNotFoundException {
        ...
        TradePair pair = new TradePair(pairCombo.getValue().split("/")[0], pairCombo.getValue().split("/")[1]);
        double requestedPositionSize = Math.min(initialBalance * riskProfileCombo.getValue().getMaxPositionSize(),
                profile.maxPositionSize());
        double leverage = riskProfileCombo.getValue().capLeverage(leverageSpinner.getValue());
        ...
        return TradeRiskContext.builder()
                ...
                .currentOpenRisk(0)
                .requestedPositionSize(requestedPositionSize)
                ...
                .expectedWinRate(probabilityCombo.getValue().getExpectedWinRate())
                ...
                .tradingSessionStatus(pair.getTradingSession() == null ? null : pair.getTradingSessionStatus())
                .tradingSessionNotes(pair.getTradingSession() == null ? "" : pair.getTradingSession().getNotes())
                ...
                .estimatedSlippagePercent(executionStrategyCombo.getValue().getEstimatedSlippage())
                .build();
    }

    private void applyTradingProfile(TradingProfile profile) {
        riskProfileCombo.setValue(profile.riskProfile());
        marketBehaviorCombo.setValue(profile.marketBehavior());
        executionStrategyCombo.setValue(profile.executionStrategy());
        liquidityCombo.setValue(profile.liquidityProfile());
        psychologyCombo.setValue(profile.psychologyProfile());
        probabilityCombo.setValue(profile.probabilityLevel());
        capitalProtectionCombo.setValue(profile.capitalProtection());
        systemDesignCombo.setValue(profile.systemDesign());

        double minimumBalance = Math.max(1000.0, profile.maxPositionSize());
        initialBalanceSpinner.getValueFactory().setValue(Math.max(initialBalanceSpinner.getValue(), minimumBalance));
        leverageSpinner.getValueFactory().setValue(profile.riskProfile().capLeverage(leverageSpinner.getValue()));
    }
    ...
    private String runSingleStrategy(String strategyName, double initialBalance, RiskDecision decision) {
        ...
        sb.append("  Final Position Size:  ").append(String.format("%.4f", decision.getFinalPositionSize()))
                .append("\n");
        ...
        sb.append("  Est. Slippage:        ").append(String.format("%.4f", decision.getEstimatedSlippage()))
                .append("%\n\n");
        ...
        sb.append("  Period:               ").append(startDatePicker.getValue()).append(" to ")
                .append(endDatePicker.getValue()).append("\n\n");
        ...
        sb.append("  Recommended Execution: ").append(decision.getRecommendedExecutionStrategy().getDisplayName())
                .append("\n");
    ...
    private void updateStatus(String message, boolean success) {
        ...
        statusLabel.setStyle(success ? "-fx-text-fill: #4CAF50; -fx-font-weight: bold;"
