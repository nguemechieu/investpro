...
function cleanup {
  ...
  if [[ $EXIT_CODE != 0 ]]; then
    ...
    if [ ! -z ${newVersion+x} ]; then
      if [ "$originMessage" == "(release) InvestPro $newVersion" ]; then
      ...
      if [ "$upstreamMessage" == "(release) InvestPro $newVersion" ]; then
...
if [ -z "${TESTFX_GITHUB_API_KEY-}" ]; then
  ...
  read -rp "Please enter your Github API key for InvestPro: " githubApiKey
else
...
if [[ -z "$upstream" ]]; then
  ...
  echo "See: https://github.com/nguemechieu/investpro/wiki/Issuing-a-Release#local-git-repository-setup"
  exit 1
fi
...
if [[ -z "$gpgKey" ]]; then
  ...
  echo "See: https://github.com/nguemechieu/investpro/wiki/Issuing-a-Release#create-a-testfx-gpg-key"
  exit 1
fi
